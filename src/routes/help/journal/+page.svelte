...
<div>
	...
	<HelpSection title="対応する請求書（売掛金の仕訳）">
		<p>
			「売掛金」を使う仕訳には、「対応する請求書」の選択欄が表示されます。取引先名が一致する発行済みの請求書から選ぶと、その仕訳が請求書に紐づき、請求書側の状態に反映されます。
		</p>
		<HelpTable
			headers={['仕訳', '請求書側の表示']}
			rows={[
				[
					'借方に売掛金（売掛金／売上高）',
					'売掛金仕訳「作成済」。請求書の「売掛金仕訳」ボタンが押せなくなる'
				],
				['貸方に売掛金（普通預金／売掛金）', '入金合計に加算され、未入金／一部入金／入金済が変わる']
			]}
		/>
		<p class="mt-2">
			紐づけると、摘要が「売掛金計上 INV-XXXX-XXXX」（借方に売掛金）または「入金
			INV-XXXX-XXXX」（貸方に売掛金）の形に自動で設定されます。既に請求書番号を含む摘要（例:「売掛金計上
			INV-XXXX-XXXX 9月分」）を書いていた場合はそのまま残ります。
		</p>
		<p class="mt-2">
			紐づけは任意です。請求書を別のシステムで作っている場合や、紐づけたくない場合は「紐づけない」のままにしてください。仕訳から売掛金の行をなくすと、紐づけは自動的に外れます。
		</p>
		<p class="mt-2">
			紐づけるときに、その請求書に既に売掛金仕訳が紐づいている、金額や取引先が請求書と異なる、といった場合は警告が表示されます。保存は止めませんので、内容を確認してください。
		</p>
		<h3 class="mt-4 mb-2 font-medium">請求書から生成した仕訳</h3>
		<p>
			請求書画面の「売掛金仕訳」ボタンから生成した仕訳は、請求書が所有します。仕訳帳では日付・取引先・摘要・明細行を編集できません（証憑の添付と行メモは編集できます）。これらを変更したい場合は請求書側で変更してください。請求書を変更すると仕訳も自動で更新されます。
		</p>
		<p class="mt-2">
			「請求書との紐づけを解除」を押すと、確認のうえで紐づけが外れ、通常の仕訳として編集できるようになります。解除後は請求書を変更しても仕訳は更新されず、請求書側では売掛金仕訳が「未作成」に戻ります。
		</p>
		<HelpNote type="info">
			<p>
				請求書画面の「売掛金入金仕訳」ボタンから生成した入金仕訳は、紐づいてはいますが請求書の所有ではありません。仕訳帳で自由に編集・削除できます。
			</p>
		</HelpNote>
	</HelpSection>
