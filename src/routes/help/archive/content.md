...
> **INFO**: 真実性の確保はアプリの機能ではなく、「訂正削除の防止に関する事務処理規程」（国税庁ひな型あり）を備え付けて運用することで満たします。詳細は[証憑管理ヘルプ](/help/evidence)を参照してください。
...
> **TIP**: 真実性の確保はタイムスタンプ付与が必須なわけではありません。国税庁ひな型の事務処理規程を備え付けて運用すれば足ります（電子帳簿保存法施行規則第4条第1項第4号）。個別の判断に迷う場合は税理士にご相談ください。
