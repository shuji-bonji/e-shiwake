	...
	<HelpSection title="電子帳簿保存法との関係">
		...
		<HelpTable
			...
			rows={[
				...
				['取引先での検索', '✅ 取引先名で検索可能'],
				['見読性', '✅ 検索HTML・帳簿HTMLで表示・印刷可能'],
				['7年間の保存', '✅ ZIPファイルとして長期保存可能'],
				['データの真実性確保', '事務処理規程の備付け・運用で対応（利用者側の措置）']
			]}
		/>
		<HelpNote type="info">
			<p>
				真実性の確保はアプリの機能ではなく、「訂正削除の防止に関する事務処理規程」（国税庁ひな型あり）を備え付けて運用することで満たします。詳細は<a
					href="{base}/help/evidence"
					class="text-primary underline">証憑管理ヘルプ</a
				>を参照してください。
			</p>
		</HelpNote>
		...
		<HelpNote type="tip">
			<p>
				真実性の確保はタイムスタンプ付与が必須なわけではありません。国税庁ひな型の事務処理規程を備え付けて運用すれば足ります（電子帳簿保存法施行規則第4条第1項第4号）。個別の判断に迷う場合は税理士にご相談ください。
			</p>
		</HelpNote>
	</HelpSection>
</div>
