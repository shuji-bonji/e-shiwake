...
<div>
	...
	<HelpSection title="ステータス管理">
		...
		<h3 class="mt-4 mb-2 font-medium">ステータスを戻す</h3>
		<p>
			操作ミスなどで進めすぎたステータスは、「ステータスを戻す」ボタンで 1
			段階前に戻せます（入金済み → 発行済み、発行済み →
			下書き）。確認ダイアログで「戻す」を選ぶと反映されます。
		</p>
		<HelpNote type="warning">
			<p>
				ステータスを戻しても、その請求書から作成した仕訳（売掛金仕訳・入金仕訳）は削除されません。仕訳も取り消す場合は、仕訳帳で該当の仕訳を削除してください。
			</p>
		</HelpNote>
	</HelpSection>
