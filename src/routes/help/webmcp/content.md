...
> **INFO**: 同じツール定義は [AI チャット](/e-shiwake/help/llm-chat)（アプリ内 LLM アシスタント）でも使われます。WebMCP はブラウザ側 AI エージェント用、AI チャットはユーザーが設定した LLM に接続するアプリ内機能です。
...
> **INFO**: UI操作型ツールは Human-in-the-Loop パターンです。AIがフォームをプリフィルし、ユーザーが内容を確認してから確定します。
...
AIがUIを操作し、ユーザーが確認・確定する Human-in-the-Loop パターンのツール群です。

> **WARNING**: UI操作型ツールは直接データを変更しません。AIがフォームに値を入力し、ユーザーが確認して「確定」ボタンを押すまで保存されません。
...
| 引数 | 型     | 必須 | 説明                                                                     |
| ---- | ------ | ---- | ------------------------------------------------------------------------ |
| page | string | ○    | ページ名（journal, ledger, trial_balance, invoice, vendors, accounts等） |

#### open_journal_editor - 仕訳入力フォーム表示

仕訳入力フォームをプリフィルして表示します。仕訳帳ページにいない場合は自動で遷移します。
...
#### set_search_query - 検索クエリセット

仕訳帳の検索ボックスにクエリをセットして検索を実行します。

| 引数  | 型     | 必須 | 説明       |
| ----- | ------ | ---- | ---------- |
| query | string | ○    | 検索クエリ |

#### confirm_delete_journal - 削除確認ダイアログ表示

指定した仕訳の削除確認ダイアログを表示します。

| 引数      | 型     | 必須 | 説明   |
| --------- | ------ | ---- | ------ |
| journalId | string | ○    | 仕訳ID |

#### open_invoice_editor - 請求書エディタ表示

請求書エディタにプリフィルデータを適用します。
