...
| 請求書 | 作成・編集・印刷、仕訳生成（売掛金計上・入金）、入金状態（未入金／一部入金／入金済）を紐づく仕訳から導出 |
...
| 機能 | 概要 |
|------|------|
| AI チャット | ユーザーが設定した LLM（ローカル/クラウド）で帳簿を自然言語操作。デスクトップはドッキングパネル、\`/chat\` で全画面 |
| WebMCP | Chrome 146+ のブラウザ側 AI エージェント連携（同一の 17 ツール） |
...
- /help/invoice/llms.txt - 請求書（作成・印刷・仕訳生成・仕訳との紐づけ）
...
主な勘定科目:
- 1001: 現金, 1002: 当座預金, 1003: 普通預金, 1004: 定期預金, 1005: 売掛金
- 2001: 買掛金, 2002: 短期借入金, 2004: 未払金, 2007: 前受金, 2008: 預り金
...
- 5001: 仕入高, 5002: 租税公課, 5003: 荷造運賃, 5004: 水道光熱費,
  ...
  5017: 地代家賃, 5018: 貸倒金, 5019: 雑損失, 5020: 雑費,
  5021: 新聞図書費, 5022: 研修費, 5023: 会議費, 5024: 支払手数料, 5025: 諸会費
...
e-shiwake は 2 系統の AI 連携に対応。どちらも同じツール定義（17ツール）を使用します。

1. **AI チャット（アプリ内コパイロット）**: ユーザーが設定した LLM（ローカル LLM / OpenAI / Anthropic / Gemini / Grok / OpenAI互換カスタム）に接続するアプリ内チャット。全ブラウザで動作。詳細: /help/llm-chat/llms.txt
2. **WebMCP（Web Model Context Protocol）**: Chrome 146+ で WebMCP が有効な場合、ブラウザ側 AI エージェントが e-shiwake のデータに直接アクセス。詳細: /help/webmcp/llms.txt
