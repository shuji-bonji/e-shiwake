		...
		<div class="rounded-lg border bg-background p-6 print:border-none print:p-0">
			...
			<div class="print-section space-y-6 {activeTab !== 'page4' ? 'hidden print:block' : ''}">
				...
				<div class="grid grid-cols-2 gap-8">
					<!-- 資産の部 -->
					<div class="space-y-4">
						<h3 class="rounded bg-muted/50 px-3 py-2 font-medium">資産の部</h3>

						{#if blueReturnData.page4.assets.current.length > 0}
							<div class="space-y-1">
								<p class="px-3 text-sm text-muted-foreground">流動資産</p>
								{#each blueReturnData.page4.assets.current as asset (asset.accountCode)}
									<div class="grid grid-cols-3 gap-2 px-3 text-sm">
										...
										<div class="text-right font-mono">{formatAmount(asset.endingBalance)}</div>
									</div>
								{/each}
							</div>
						{/if}

						{#if blueReturnData.page4.assets.fixed.length > 0}
							<div class="space-y-1">
								<p class="px-3 text-sm text-muted-foreground">固定資産</p>
					...
					<!-- 負債・資本の部 -->
					<div class="space-y-4">
						<h3 class="rounded bg-muted/50 px-3 py-2 font-medium">負債・資本の部</h3>

						{#if blueReturnData.page4.liabilities.current.length > 0 || blueReturnData.page4.liabilities.fixed.length > 0}
							<div class="space-y-1">
								<p class="px-3 text-sm text-muted-foreground">負債</p>
								{#each [...blueReturnData.page4.liabilities.current, ...blueReturnData.page4.liabilities.fixed] as liability (liability.accountCode)}
									<div class="grid grid-cols-3 gap-2 px-3 text-sm">
										<div>{liability.accountName}</div>
										<div class="text-right font-mono text-muted-foreground">
											{formatAmount(liability.beginningBalance)}
										</div>
										<div class="text-right font-mono">{formatAmount(liability.endingBalance)}</div>
									</div>
								{/each}
							</div>
						{/if}
						...
						<div class="grid grid-cols-3 gap-2 border-t px-3 pt-2">
							<div class="font-medium">負債・資本合計</div>
							<div class="text-right font-mono text-muted-foreground">
								{formatAmount(
				...
				<!-- 貸借バランス確認 -->
				<div
					class="rounded-md p-4 {blueReturnData.page4.isBalanced
						? 'bg-green-50 dark:bg-green-950'
						: 'bg-red-50 dark:bg-red-950'}"
				>
					<p
						class="text-sm font-medium {blueReturnData.page4.isBalanced
