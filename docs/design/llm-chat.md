...
> **追記（2026-07-04）**: v0.5.0 で本設計の Phase 1〜4 を実装・リリースしたことに伴い、e-shiwake-ai は**アーカイブ**とした。SQLite が本番データ（IndexedDB）と同期しない構造的制約があり、メンテナンスも停止していたため。ブラウザ外エージェントの経路は将来的に WebMCP の標準化で代替する。個人事業主向けというより VPC 内 DB + stdio MCP を要する企業向けの構成であり、必要になった際は参照実装として復活させる。
