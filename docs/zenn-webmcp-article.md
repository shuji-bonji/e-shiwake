...
```json
{
	...
	"vendor": "東京タクシー",
	"debitLines": [{ "accountCode": "5005", "amount": 3200, "taxCategory": "purchase_10" }],
	"creditLines": [{ "accountCode": "1001", "amount": 3200, "taxCategory": "na" }]
}
```
...
const searchJournalsTool: WebMCPToolDefinition = {
	...
	inputSchema: {
		...
		properties: {
			query: {
				type: 'string',
				description: '検索クエリ。例: "Amazon", "消耗品費", "10000", "2025-01"'
			},
			fiscalYear: {
				type: 'number',
	...
	execute: async (input) => {
		...
		// IndexedDB から仕訳を取得
		let journals = fiscalYear ? await getJournalsByYear(fiscalYear) : await getAllJournals();
...
export function dispatchJournalChange(action: 'create' | 'delete', journalId: string): void {
