...
| `/vendors`       | 取引先管理                            |
| `/accounts`      | 勘定科目管理                          |
| `/data`          | データ管理（バックアップ/リストア/エクスポート） |
| `/archive`       | アーカイブ（年度決算パッケージ生成・リストア）   |
...
1. サイドバー → 「データ管理」をクリック
2. **フルバックアップ**:
   - 「フルバックアップ」セクションの「バックアップを作成」ボタンをクリック
   - 全年度のデータ（仕訳+勘定科目+取引先+固定資産+請求書+設定+証憑PDF）がZIPで保存される
3. **フルリストア**:
   - 「リストア」セクションでバックアップZIPファイルを選択
   - プレビュー表示で内容を確認
   - 「リストア実行」で全データを上書き復元

## エクスポート

1. データ管理ページのエクスポートセクション
2. 対象年度を選択
3. 形式を選択:
   - **JSON**: 仕訳データ出力
   - **CSV**: 仕訳データ出力（他ソフト連携用）
...
2. **アーカイブ作成**:
   - 対象年度を選択
   - 「アーカイブを作成」ボタンでZIPを生成（仕訳+証憑PDF）
   - 作成後、年度データを削除してブラウザ容量を節約可能
3. **アーカイブリストア**:
   - 「リストア」タブでアーカイブZIPを選択
   - 検索機能で仕訳を確認可能
   - 「リストア実行」でデータを復元
