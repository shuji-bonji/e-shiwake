...
1. サイドバー → データ管理（`/data`）
2. **フルバックアップ**: 全年度のデータ（仕訳+勘定科目+取引先+固定資産+請求書+設定+証憑）をZIPで保存
3. **フルリストア**: バックアップZIPから全データを上書き復元
4. **エクスポート**: 年度別にJSON / CSV形式で出力

### アーカイブ

1. サイドバー → アーカイブ（`/archive`）
2. **アーカイブ作成**: 年度決算パッケージ（仕訳+証憑）をZIPで生成、年度データを削除してブラウザ容量を節約
3. **アーカイブリストア**: ZIPからデータを復元（検索機能付き）
...
7. **フルバックアップ**: `/data` でフルバックアップZIPを作成（全データ+証憑）
8. **アーカイブ作成**: `/archive` で確定した年度のアーカイブZIPを生成（任意）
